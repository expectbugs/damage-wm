...
Everything runs against the Postgres database `g2cc` and the Qdrant collection
`g2cc_music`. The names are history, not a dependency: Damage took the music
system over in place and **is the only writer of the music tables from now on**.
G2CC on disk is read-only to us.

---

## Running it

There is no Damage-owned venv yet. Use G2CC's (config key `musicPython`, default
`/home/user/G2CC/audio/venv/bin/python`) and run **from this directory** —
`-m enrich.…` and `-m viz` resolve against the working directory:
...
`Enrich.kt` runs the first and last of those for every newly grabbed track, in
this order:
...
A failed pass is logged with the head of its stderr and the chain continues —
a track that plays never stops playing because a web service was unreachable.
...
`~/.damage/config.json` (`MUSIC.md` §9.7) — the file the Kotlin `Config` reads:
...
unreadable or malformed one raises rather than quietly enriching the wrong files.

Environment overrides: `DAMAGE_ENRICH_TMP` (scratch directory for decode
temporaries), `CLAUDE_CLI`, `DAMAGE_CLAUDE_MODEL` / `DAMAGE_CLAUDE_EFFORT` (the
profile pass's one-shot), `DAMAGE_PARAKEET_DIR` (the ASR engine the `speech`
pass borrows from G2CC's dictation pipeline — the one thing that did not move
with the takeover).
...
Decodes to mono 22050 Hz with librosa (ffmpeg to a temp wav when librosa cannot
open the container), then emits **DVIZ v1**:

* a **24-band log-spaced spectrum envelope at 20 fps**, 4-bit levels — 40 Hz to
  just under Nyquist, pink-compensated (+3 dB/octave) so the top bands are not
  permanently dark, mapped through a per-track percentile window so a quiet
  track uses the full 0–15 range;
* an **RMS envelope per 20 ms**, 4-bit, mapped the same way;
* **beat times in ms** from `librosa.beat.beat_track`.

The byte layout is fixed by `VizData` in `core/.../windows/music/MusicModel.kt`:
magic `DVIZ`, u8 version, u8 fps, u8 bands, u32 LE frames / rmsCount / beats,
then frames×bands nibbles packed high-first row-major, the rms nibbles, and the
beat times as u32 LE. `VizData.encode()` writes exactly these bytes and
`VizData.decode()` reads them.

No smoothing, gravity or peak-hold — those belong to the renderers (`Viz.kt`),
so the stored data stays the measurement and a renderer change needs no
re-analysis.

**Measured**, not modeled (2026-09-02, this box): a 6.5-minute FLAC takes 5.5 s
and produces a 107 KB blob; roughly 16 KB per minute of audio. Nothing is
truncated — the whole track is analysed however long it is.

Failures are loud: a reason on stderr and a non-zero exit (2 = bad arguments or
a missing file, 1 = the analysis failed). Nothing is written to stdout on a
failure.
...
No pytest, no network, no database. It synthesizes fixtures with ffmpeg in a
temp directory, runs `viz.py` the way `Enrich.kt` does, and decodes the result
with its own reader written from the `VizData` layout — so the two encoders
have to agree on the bytes. The Kotlin side of the bridge is
...
`requirements-frozen.txt` is a `pip freeze` of `/home/user/G2CC/audio/venv` as
of 2026-09-02 (224 packages; Python 3.13.14, librosa 0.11.0, numpy 2.4.4,
psycopg 3.3.4, torch 2.12.0, transformers 4.57.6). A record of what these
passes ran against, not an install list — a Damage-owned venv is a later chore,
and most of the 224 packages belong to G2CC's other tooling.
