...
| 🔴 **The glasses NEVER send `RingRawData` on sid 0x91** — the stock service accepts only `commandId=EVENT` (the ring-MAC registration) and rejects `RAW_DATA`; no firmware path fills the rawData field. The vendor schema defines a feature the glasses firmware does not implement | **C+M** | openCFW `pb_service_ring.c` (instruction-level recovery of our 2.2.6.10 base: the whole 0x91 service is decode → accept EVENT only → echo the EVENT) **+** zero rawData frames across both captures in either direction (M) **+** G2CC docs §10/§11: the official app reads ring battery on the ring's OWN link (`DeviceStatus.batteryLevel`). Waiting on the relay was waiting for a message the firmware cannot produce; ring battery requires the phone's own link to the ring (the Faceclaw-proven second connection) |
| The ring's phone link is a **vendor GATT service `bae80001-…`** (notify `bae80011`/`bae80013`, write `bae80010`/`bae80012`), **no standard Battery Service, no battery in the advertisement** | **M** | APK 0.14 `RingProbe` on hardware 2026-08-31 (device-log): full GATT enumeration + parsed advertisement (mfr 0x5245 = reversed MAC + serial only) |
| The ring's phone link is **REQUEST/RESPONSE, not push** — a phone secondary gets no unsolicited frames | **M** | subscribed both notify chars + tapped the ring → **zero frames** in 45 s; every notify in the captures follows a phone write. Gestures go ring→glasses (the bonded primary), navigation ring→glasses→`e0-01` (G2CC §11) |
| The vendor frame's trailing 2 bytes are **not a standard CRC-16** | **M** | offline scan of all 14 common CRC-16 variants × every body range/endianness over 47 clean capture frames: no match. The 4-byte leading field is not a CRC-32 of the body either. Custom checksum — uncracked; polling the ring is blocked on it |
