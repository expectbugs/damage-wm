...
| **The cached draws 13/14/15 are FLAT: the mode byte's "lenses differ" high bit is ignored and one (x, y) draws into each lens's shadow** — so a cached draw can only sit at disparity 0 | **V** | `zlib_glue.c` header ("most modes ignore it; for mode 3 … for mode 9 …") and the mode 13/14/15 dispatch, which passes one `src` to `cfw_texture_draw_*` on the arm's own shadow. Recorded 2026-09-06 (`HANDOFF.md` §40.6): this is why the cache serves plane 0 only. A per-lens variant is a firmware-side ask |
