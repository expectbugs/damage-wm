...
| The journal's slow regime (08-31 13:00 →, `HANDOFF.md` §31.6) is the PHONE's radio path, the fast one PC-direct BlueZ | **C** | `damage.log` "driving via remote:aphone" + the journal's own "no done from aphone" stall notes in the slow hours + the §11–§13 timeline; not M because the journal carried no transport field — it does since §32 (`via`) |
