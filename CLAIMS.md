...
| The two 165,888-byte EvenHub container buffers cannot live in arena 27 (184,320 B); arena 13 (839,808 B) or arena 20 (460,800 B) holds them | **I** | arena sizes (a5d1c31 `malloc.h`, `debug.c`); the allocation call is not isolated. The overlay's `f13/20/27` readout settles it (M0.1) |
...
| The stock KV store keeps a boot counter (`kvbooCount`) that each start reads, increments and writes back | **V** (openCFW) | `g2-service-kvdb-recovery.md` (RAM `0x20074988`); a source for the hold-back rule's boot count without a new write (`FIRMWARE.md` §3) |
